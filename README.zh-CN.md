...
根入口和 `.AGENTS/` 可以独立使用，不依赖本仓库的 README 和贡献指南。这些文件用于解释约定，可按项目需要保留或替换。默认角色和技能不预设语言、框架或构建命令；按需求调整即可。
...
根 `AGENTS.md` 保存公共项目背景和约定。每个角色用自己的 `AGENTS.md` 描述职责，用 `memory/MEMORY.md` 保存当前知识，用 `skills/` 保存可复用方法。可选的 `local/` 资料由 Git 忽略，属于当前工作实例。
...
维护本模板仓库时，参见[贡献指南](CONTRIBUTING.md)。
